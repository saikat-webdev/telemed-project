    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto">
            <form method="GET" class="flex gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search patient or message" class="rounded-xl border-gray-200 focus:border-violet-500 focus:ring-violet-500">
            ...
            <a href="{{ route('doctor.appointments.index') }}" class="bg-violet-600 hover:bg-violet-700 text-white px-5 py-2.5 rounded-xl font-semibold shadow-sm transition-all text-center">
                View Appointments
            </a>
        </div>
    </div>
